...
def main():
    ...
    #                 )
        ...
        #             proc = subprocess.run([pbessolve, pbes_file], stdout=subprocess.PIPE, text=True, check=True)
        ...
        #     with open(os.path.join(args.output, "solution.json"), "a", encoding="utf-8") as f:
        #         json.dump(result, f)
        #         f.write("\n")
            ...
            for result in results:
                ...
                if name in result["file"]:
                    ...
                    for key, value in expected[0].items():
                        ...
                        if value != actual[key]:
                            logger.error(f"Verification failed for {name}. Mismatch for product {key}. Expected: {value}, Actual: {actual[key]}")
                            continue
